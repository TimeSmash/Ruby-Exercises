...
The variables donovan_grade and donovan_keys simply show how to get the value or key from the object (In some hashes, many key-value pairs 
are listed at once, and the .values() and `.keys()` methods create arrays of values or keys from those pairs. 
...
we create a method failures which takes a failing_array as an argument, starting off by headlining a message of who failed. 
